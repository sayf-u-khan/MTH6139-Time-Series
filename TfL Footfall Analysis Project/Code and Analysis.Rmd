...
This report presents a time-series analysis of passenger entry data for Chadwell Heath station, sourced from Transport for London (TfL). The primary objective was to model historical trends and develop a predictive forecast for future passenger footfall. Using Meta's Prophet model in R, the analysis successfully identified strong weekly and yearly seasonality corresponding with commuter behaviour. The final forecast demonstrated a high degree of accuracy, and the overall project was awarded a grade of 95%.
...
library(dplyr)
library(forecast)
ChadwellHeath = readxl::read_excel("data/Chadwell Heath Footfall Data (Cleaned).xlsx")
```
...
```{r}
plot(entries.meta, entries.predictions, type = 'l', xlab = 'Years', ylab = 'Daily Entries')
```
